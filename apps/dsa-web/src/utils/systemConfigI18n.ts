...
const fieldDescriptionMap: Record<string, string> = {
  ...
  SCHEDULE_TIME: '每日定时任务执行时间，格式为 HH:MM（24 小时制）。',
  SCHEDULE_ENABLED: '启用后系统将按设定的时间每天自动执行股票分析并推送结果。',
  SCHEDULE_RUN_IMMEDIATELY: '定时模式启动时是否立即执行一次全量分析。',
