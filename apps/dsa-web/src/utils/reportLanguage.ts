import type { ReportLanguage } from '../types/analysis';

export const normalizeReportLanguage = (value?: string | null): ReportLanguage =>
  value === 'en' ? 'en' : 'zh';

const REPORT_TEXT = {
  zh: {
    keyInsights: '核心洞察',
    noAnalysisSummary: '暂无分析结论',
    actionAdvice: '操作建议',
    noAdvice: '暂无建议',
    trendPrediction: '趋势预测',
    noPrediction: '暂无预测',
    marketSentiment: '市场情绪',
    strategyPoints: '策略点位',
    sniperLevels: '狙击点位',
    idealBuy: '理想买入',
    secondaryBuy: '二次买入',
    stopLoss: '止损价位',
    takeProfit: '止盈目标',
    noValue: '—',
    newsFeed: '资讯动态',
    relatedNews: '相关资讯',
    refresh: '刷新',
    retry: '重试',
    dismiss: '关闭',
    details: '查看详情',
    loadingNews: '加载资讯中...',
    noNews: '暂无相关资讯',
    noNewsDescription: '可稍后刷新以获取最新资讯。',
    openLink: '跳转',
    transparency: '透明度',
    traceability: '数据追溯',
    rawResult: '原始分析结果',
    analysisSnapshot: '分析快照',
    copy: '复制',
    copied: '已复制',
    recordId: '记录 ID',
    fullReport: '完整分析报告',
    loadingReport: '加载报告中...',
    loadReportFailed: '加载报告失败',
    copyMarkdownSource: '复制 Markdown 源码',
    copyPlainText: '复制纯文本',
    downloadPdf: '下载 PDF',
    downloadingPdf: '正在生成 PDF...',
    downloadPdfFailed: 'PDF 下载失败，请稍后重试',
    shareReport: '分享',
    sharingReport: '正在生成分享链接...',
    shareLinkCopied: '分享链接已复制，可直接发给朋友',
    shareFailed: '分享失败，请稍后重试',
    sharedReportBadge: '公开分享报告',
    sharedReportDisclaimer: '本报告由用户分享，仅供研究参考，不构成投资建议。',
    loginToAnalyze: '登录 DSA 开始分析',
    analysisModel: '分析模型',
    predictionCycleTitle: '预测周期',
    predictionCycleAnchor: '周期锚点',
    predictionCycleTarget: '预测目标',
    predictionCycleDataAsOf: '数据截至',
    predictionCycleCached: '复用本周期缓存',
    predictionCycleProbeCredits: '探测积分 -{credits}',
    predictionCycleHint: '同一预测周期内共用 canonical 分析；有新情报时将自动重分析。',
    fearGreedIndex: '恐惧贪婪指数',
    boardLinkage: '板块联动',
    relatedBoards: '关联板块',
    leadingBoard: '领涨',
    laggingBoard: '领跌',
    neutralBoard: '中性',
    companyBasics: '基本信息',
    viewCompanyBasics: '查看基本信息',
    fullName: '公司全称',
    industry: '所属行业',
    listingDate: '上市日期',
    totalShareCapital: '总股本',
    floatShareCapital: '流通股本',
    employeeCount: '员工规模',
    website: '公司官网',
    companyIntro: '公司基本介绍',
    coreManagement: '核心管理层',
    legalRepresentative: '法定代表人',
    generalManager: '总经理',
    boardSecretary: '董秘',
  },
  en: {
    keyInsights: 'KEY INSIGHTS',
    noAnalysisSummary: 'No analysis summary yet',
    actionAdvice: 'Action Advice',
    noAdvice: 'No advice yet',
    trendPrediction: 'Trend Outlook',
    noPrediction: 'No forecast yet',
    marketSentiment: 'Market Sentiment',
    strategyPoints: 'STRATEGY POINTS',
    sniperLevels: 'Action Levels',
    idealBuy: 'Ideal Entry',
    secondaryBuy: 'Secondary Entry',
    stopLoss: 'Stop Loss',
    takeProfit: 'Take Profit',
    noValue: '—',
    newsFeed: 'NEWS FEED',
    relatedNews: 'Related News',
    refresh: 'Refresh',
    retry: 'Retry',
    dismiss: 'Close',
    details: 'View details',
    loadingNews: 'Loading news...',
    noNews: 'No related news',
    noNewsDescription: 'Refresh later to check for the latest updates.',
    openLink: 'Open',
    transparency: 'TRANSPARENCY',
    traceability: 'Data Traceability',
    rawResult: 'Raw Analysis Result',
    analysisSnapshot: 'Analysis Snapshot',
    copy: 'Copy',
    copied: 'Copied!',
    recordId: 'Record ID',
    fullReport: 'Full Analysis Report',
    loadingReport: 'Loading report...',
    loadReportFailed: 'Failed to load report',
    copyMarkdownSource: 'Copy Markdown Source',
    copyPlainText: 'Copy Plain Text',
    downloadPdf: 'Download PDF',
    downloadingPdf: 'Generating PDF...',
    downloadPdfFailed: 'Failed to download PDF. Please try again.',
    shareReport: 'Share',
    sharingReport: 'Creating share link...',
    shareLinkCopied: 'Share link copied. Send it to a friend.',
    shareFailed: 'Failed to share. Please try again.',
    sharedReportBadge: 'Public shared report',
    sharedReportDisclaimer: 'This report was shared by a user for research only; not investment advice.',
    loginToAnalyze: 'Log in to DSA to analyze stocks',
    analysisModel: 'Model',
    predictionCycleTitle: 'Prediction Cycle',
    predictionCycleAnchor: 'Anchor Date',
    predictionCycleTarget: 'Target Date',
    predictionCycleDataAsOf: 'Data As Of',
    predictionCycleCached: 'Cached This Cycle',
    predictionCycleProbeCredits: 'Probe Credits -{credits}',
    predictionCycleHint: 'Canonical analysis is shared within the same prediction cycle; new intel triggers a refresh.',
    fearGreedIndex: 'Fear & Greed Index',
    boardLinkage: 'BOARD LINKAGE',
    relatedBoards: 'Related Boards',
    leadingBoard: 'Leading',
    laggingBoard: 'Lagging',
    neutralBoard: 'Neutral',
    companyBasics: 'Company Basics',
    viewCompanyBasics: 'View Company Basics',
    fullName: 'Full Name',
    industry: 'Industry',
    listingDate: 'Listing Date',
    totalShareCapital: 'Total Shares',
    floatShareCapital: 'Float Shares',
    employeeCount: 'Employees',
    website: 'Website',
    companyIntro: 'Company Introduction',
    coreManagement: 'Core Management',
    legalRepresentative: 'Legal Representative',
    generalManager: 'General Manager',
    boardSecretary: 'Board Secretary',
  },
} as const;

export const getReportText = (language?: string | null) => REPORT_TEXT[normalizeReportLanguage(language)];
