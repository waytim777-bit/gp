...
如果系统提供了历史分析记录 ID（record_id），你应优先调用 `get_report_context` 复用同一次分析上下文（包含 K 线、筹码、新闻等裁剪后的数据）。仅当复用数据缺失或过期时，再调用其他工具进行补全。
    ...
    def chat(self, message: str, session_id: str, progress_callback: Optional[Callable] = None, context: Optional[Dict[str, Any]] = None) -> AgentResult:
        ...
        if context:
            ...
            if context.get("record_id"):
                context_parts.append(f"历史分析记录ID(record_id): {context['record_id']}")
