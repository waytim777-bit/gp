...
- [改进] Web 我的订阅页新增添加订阅弹窗，按设计稿展示股票输入、推送间隔选择和积分预估，并接入创建订阅流程。
- [修复] 修复系统配置接口返回分析类配置时的 schema 校验失败。
- [修复] 历史记录删除会解除自动共享缓存、公开分享和购买副本引用，避免已分析记录因外键约束删除失败；已推荐到预测报告市场的记录返回 409 提示先下架。
- [改进] Web 首页分析任务卡片优化进度消息显示，隐藏消息前缀中的股票名称/代码并缩小任务状态标签。
- [改进] Web 首页分析任务卡片按 Figma 设计稿调整暗色背景、主色描边、进度条和字号。
- [改进] Web 完整分析报告的财务、技术、K 线和关键支撑/阻力区块在紧凑模式下统一使用 Card 容器背景。
- [改进] Web 分析报告营收增长与盈利能力图表统一使用主题主色 `--primary`。
- [改进] Web 报告业务模式区块改用统一 Card 容器展示。
